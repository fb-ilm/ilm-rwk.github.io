...
// Arreglo temporal para opciones dinámicas de tbRetrabajos
...
function cancelarCrearPin() {
  ...
  document.getElementById("lbl-admin-pin").textContent =
  ...
  document.getElementById("btn-admin-submit").textContent = "Acceder al Panel";
  ...
  setInterval(() => {
    if (sesionActiva) {
      ...
      const restanteMin = Math.max(
        0,
        Math.ceil((TIMEOUT_INACTIVIDAD_MS - transcurrido) / 60000),
      );
      document.getElementById("lbl-timer").textContent =
        `Inactividad: ${restanteMin}m`;
...
function intentarCerrarSesion(porInactividad = false) {
  const pendientes = JSON.parse(
    localStorage.getItem(CACHE_KEY_RETRABAJOS) || "{}",
  );
...
function mostrarWorkspace() {
  ...
  document.getElementById("lbl-operador").textContent =
    sesionActiva.employee_id;
...
document.getElementById("form-login-op").addEventListener("submit", (e) => {
  ...
  // 1. Validación Número de Empleado (Inicia en 0, termina en A, 7 caracteres exactos)
  const empRegex = /^0[a-zA-Z0-9]{5}[aA]$/;
  if (!empRegex.test(empInput)) {
    alert(
      "Número de empleado inválido.\n\nRequisitos:\n• Debes escanear tu gafete.",
    );
    document.getElementById("login-emp").focus();
    return;
  }

  // 2. Validación de Estación (Inicia con ROL y contiene exactamente o al menos dos '_')
  const estUpper = estInput.toUpperCase();
  const estRegex = /^ROL.*_.*_.*$/;
  // Contar cuántos guiones bajos tiene
  const countGuiones = (estInput.match(/_/g) || []).length;

  if (!estRegex.test(estUpper) || countGuiones !== 2) {
    alert(
      "Estación de trabajo inválida.\n\nRequisitos:\n• Debes escanear tu estación de trabajo.",
    );
    document.getElementById("login-est").focus();
    return;
  }

  // Si pasa las validaciones, iniciar sesión normal
  sesionActiva = {
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    if (pin.length !== 6 || isNaN(pin))
      return alert("El PIN debe tener 6 dígitos numéricos.");

    if (creandoNuevoPin) {
      const pinConfirm = document
        .getElementById("admin-pin-confirm")
        .value.trim();
    ...
    try {
      ...
      if (res.success) {
        ...
        document.getElementById("lbl-admin-pin").textContent =
          "Crear Nuevo PIN (6 dígitos):";
        document.getElementById("grp-admin-pin-confirm").style.display =
          "block";
        document.getElementById("admin-pin-confirm").required = true;
        document.getElementById("btn-admin-submit").textContent =
          "Guardar PIN y Acceder";
        document.getElementById("btn-admin-cancel-create").style.display =
          "block";
        document.getElementById("admin-pin").value = "";
        document.getElementById("admin-pin-confirm").value = "";
      } else {
...
async function cargarCatalogos(forzar = false) {
  ...
  try {
    showLoading(true);
    const res = await (
      await fetch(`${GAS_API_URL}?action=getCatalogos`)
    ).json();
    if (res.success) {
      catalogos = res.data;
...
function poblarSelects() {
  ...
  (catalogos.productos || []).forEach(
    (p) =>
      (selModel.innerHTML += `<option value="${p.id_product}">${p.text_product}</option>`),
  );
  ...
  (catalogos.categorias || []).forEach(
    (c) =>
      (selCatFiltro.innerHTML += `<option value="${c.id_category}">${c.name_category}</option>`),
  );
}

/**
 * 5. MANTENIMIENTO INTUITIVO (CRUD NO TÉCNICO)
 */
...
function obtenerDataTablaActiva(sheetName) {
  ...
  if (sheetName === "tbRetrabajos")
    return catalogos.tbRetrabajos || catalogos.retrabajos || [];
...
function renderizarTablaCRUD() {
  ...
  if (sheetName === "tbProductos") {
    thead +=
      '<th>Modelo / Producto</th><th style="text-align:center;">Acción</th></tr></thead>';
    if (datos.length === 0)
      tbody +=
        '<tr><td colspan="2" style="text-align:center; color:var(--text-muted);">Sin modelos registrados.</td></tr>';
    datos.forEach((row) => {
      tbody += `<tr>
    ...
    thead +=
      '<th>Defecto</th><th>Sección</th><th>Tipo de Problema</th><th style="text-align:center;">Acción</th></tr></thead>';
    if (datos.length === 0)
      tbody +=
        '<tr><td colspan="4" style="text-align:center; color:var(--text-muted);">Sin defectos registrados.</td></tr>';
    datos.forEach((row) => {
    ...
    thead +=
      '<th>Acción de Reparación</th><th>Problema</th><th>Tipo de Respuesta</th><th>Opciones</th><th style="text-align:center;">Acción</th></tr></thead>';
    if (datos.length === 0)
      tbody +=
        '<tr><td colspan="5" style="text-align:center; color:var(--text-muted);">Sin acciones registradas.</td></tr>';
    datos.forEach((row) => {
      const typeLabel =
    ...
    thead +=
      '<th>Categoría</th><th>Descripción / Validación</th><th style="text-align:center;">Acción</th></tr></thead>';
    if (datos.length === 0)
      tbody +=
        '<tr><td colspan="3" style="text-align:center; color:var(--text-muted);">Sin categorías registradas.</td></tr>';
    datos.forEach((row) => {
      tbody += `<tr>
    ...
    thead +=
      '<th>Componente</th><th>Problemas Aplicables</th><th style="text-align:center;">Acción</th></tr></thead>';
    if (datos.length === 0)
      tbody +=
        '<tr><td colspan="3" style="text-align:center; color:var(--text-muted);">Sin secciones registradas.</td></tr>';
    datos.forEach((row) => {
      tbody += `<tr>
    ...
    thead +=
      '<th>Nombre</th><th>Correo</th><th>Rol</th><th style="text-align:center;">Acción</th></tr></thead>';
    if (datos.length === 0)
      tbody +=
        '<tr><td colspan="4" style="text-align:center; color:var(--text-muted);">Sin usuarios registrados.</td></tr>';
    datos.forEach((row) => {
      tbody += `<tr>
...
function abrirModalNuevaFila() {
  ...
  if (sheetName === "tbProductos") {
    ...
    let optionsProb =
      '<option value="">-- Selecciona el problema al que entra --</option>';
    (catalogos.problemas || []).forEach((p) => {
...
/**
 * Guardado con Auto-generación de IDs y Validación de Duplicados
 */
document
  .getElementById("form-crud-add")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    const sheetName = document.getElementById("select-crud-table").value;
    ...
    if (sheetName === "tbProductos") {
      ...
      const yaExiste = datosActuales.some(
        (p) =>
          p.text_product.toString().trim().toUpperCase() ===
          nombre.toUpperCase(),
      );
      if (yaExiste)
        return alert(`El modelo "${nombre}" ya existe en la base de datos.`);

      const maxId = datosActuales.reduce(
        (max, p) => Math.max(max, parseInt(p.id_product) || 0),
        0,
      );
      payloadEnvio = {
        id_product: (maxId + 1).toString(),
        text_product: nombre,
      };
    } else if (sheetName === "tbCodigos") {
      ...
      if (checkedProbs.length === 0)
        return alert(
          "Selecciona al menos un tipo de problema para este defecto.",
        );

      let maxId = datosActuales.reduce(
        (max, c) => Math.max(max, parseInt(c.id_code) || 0),
        0,
      );
      const rowsToAdd = [];
      ...
      for (const idProb of checkedProbs) {
        const yaExiste = datosActuales.some(
          (c) =>
            c.text_code.toString().trim().toUpperCase() ===
              defecto.toUpperCase() &&
      ...
      if (rowsToAdd.length === 0)
        return alert(
          "Este defecto ya está registrado para esa sección y problema(s).",
        );
      ...
      if (ansType === "CHOOSE") {
        if (listaOpcionesRetrabajoTemporal.length === 0)
          return alert("Agrega al menos una opción a la lista desplegable.");
        optionsFinal = listaOpcionesRetrabajoTemporal.join(" | ");
      }

      const yaExiste = datosActuales.some(
        (r) =>
          r.text_rework.toString().trim().toUpperCase() ===
            accion.toUpperCase() &&
          r.id_problem.toString() === idProb.toString(),
      );
      if (yaExiste)
        return alert(
          `La acción "${accion}" ya está registrada para este problema.`,
        );

      const maxId = datosActuales.reduce(
        (max, r) => Math.max(max, parseInt(r.id_rework) || 0),
        0,
      );
      payloadEnvio = {
      ...
      const nombreCat = document
        .getElementById("crud-name_category")
        .value.trim();
      const question = document
        .getElementById("crud-question_category")
        .value.trim();
      const yaExiste = datosActuales.some(
        (c) =>
          c.name_category.toString().trim().toUpperCase() ===
          nombreCat.toUpperCase(),
      );
      if (yaExiste) return alert(`La categoría "${nombreCat}" ya existe.`);

      const maxId = datosActuales.reduce(
        (max, c) => Math.max(max, parseInt(c.id_category) || 0),
        0,
      );
      payloadEnvio = {
        id_category: (maxId + 1).toString(),
        name_category: nombreCat,
        question_category: question,
      };
    } else if (sheetName === "tbSecciones") {
      const nombreSec = document
        .getElementById("crud-name_section")
        .value.trim();
      const checked = Array.from(
        document.querySelectorAll('input[name="crud-sec-prob"]:checked'),
      ).map((el) => el.value);
      const applyStr = checked.length ? checked.join(" | ") : "0";

      const yaExiste = datosActuales.some(
        (s) =>
          s.name_section.toString().trim().toUpperCase() ===
          nombreSec.toUpperCase(),
      );
      if (yaExiste) return alert(`La sección "${nombreSec}" ya existe.`);

      const maxId = datosActuales.reduce(
        (max, s) => Math.max(max, parseInt(s.id_section) || 0),
        0,
      );
      payloadEnvio = {
        id_section: (maxId + 1).toString(),
        name_section: nombreSec,
        apply_problem: applyStr,
      };
    } else if (sheetName === "tbUsuarios") {
      const nombreUser = document.getElementById("crud-name_user").value.trim();
      const email = document
        .getElementById("crud-email_user")
        .value.trim()
        .toLowerCase();
      const rol = document.getElementById("crud-rol_user").value;

      const yaExiste = datosActuales.some(
        (u) => u.email_user.toString().trim().toLowerCase() === email,
      );
      if (yaExiste) return alert(`El correo "${email}" ya está registrado.`);

      const maxId = datosActuales.reduce(
        (max, u) => Math.max(max, parseInt(u.id_user) || 0),
        0,
      );
      payloadEnvio = {
        id_user: (maxId + 1).toString(),
        name_user: nombreUser,
        email_user: email,
        rol_user: rol,
        pass_user: "",
      };
    ...
    try {
      const res = await (
        await fetch(GAS_API_URL, {
          ...
          headers: { "Content-Type": "text/plain;charset=utf-8" },
          body: JSON.stringify({
            action: "agregarFilaTabla",
            sheetName: sheetName,
            newRowObj: payloadEnvio,
          }),
        })
      ).json();
...
/**
 * 6. ESCANEO Y DETECCIÓN DE PIEZA RECHAZADA
 */
...
function iniciarInspeccion() {
  document.getElementById("lbl-shopfloor").textContent =
    currentPiece.shopfloor_id;
  const modelObj = (catalogos.productos || []).find(
    (p) => p.id_product == currentPiece.id_model,
  );
  document.getElementById("lbl-modelo").textContent = modelObj
    ? modelObj.text_product
    : currentPiece.id_model;
  document.getElementById("modal-pieza").classList.remove("active");
}
...
/**
 * 7. SOLUCIÓN DE RETRABAJO (tbRetrabajos)
 */
function abrirModalSolucionRetrabajo() {
  ...
  const idProbGuardado = dataPieza.id_problem
    ? dataPieza.id_problem.toString().trim()
    : "";
  ...
  let opcionesFiltradas = listaRetrabajos.filter(
    (r) =>
      r.id_problem &&
      idProbGuardado &&
      r.id_problem.toString().trim() === idProbGuardado,
  );
  ...
  selAccion.innerHTML =
    '<option value="">-- Selecciona Acción Realizada --</option>';
...
document
  .getElementById("select-accion-retrabajo")
...
document
  .getElementById("form-solucion-retrabajo")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    const selAccion = document.getElementById("select-accion-retrabajo");
    ...
    try {
      ...
      if (res.success) {
        ...
        document
          .getElementById("modal-solucion-retrabajo")
...
document.querySelectorAll(".comp-zone").forEach((zone) => {
  zone.addEventListener("click", () => {
    document
      .querySelectorAll(".comp-zone")
      .forEach((z) => z.classList.remove("activo"));
...
document.getElementById("form-categorias").addEventListener("submit", (e) => {
  e.preventDefault();
  const checked = Array.from(
    document.querySelectorAll('input[name="cat-choice"]:checked'),
  );
...
/**
 * 8. CLASIFICACIÓN DE DEFECTO (RETRABAJO)
 */
function iniciarFlujoProblema() {
  ...
  document.getElementById("select-codigo").innerHTML =
...
document.getElementById("select-problema").addEventListener("change", (e) => {
  ...
  if (!probId) {
    ...
    return (selCod.innerHTML =
      '<option value="">-- Selecciona primero el tipo --</option>');
  }

  // Filtrar estrictamente por Tipo de Problema Y por Sección seleccionada
  const codigosFiltrados = (catalogos.codigos || []).filter((c) => {
    const matchProblem =
      c.id_problem &&
      c.id_problem.toString().trim() === probId.toString().trim();
    const matchSection =
      c.id_section &&
      c.id_section.toString().trim() === currentSectionId.toString().trim();
  ...
  if (codigosFiltrados.length === 0) {
    selCod.innerHTML =
      '<option value="">No hay defectos para esta sección y problema</option>';
  } else {
...
document.getElementById("form-problema").addEventListener("submit", (e) => {
  ...
  retrabajoData.otro_detalle = document
    .getElementById("input-otro-defecto")
    .value.trim();
...
/**
 * 9. PREGUNTAS DINÁMICAS (LÓGICA IMÁGENES ID 3 + TRIGGER)
 */
function procesarSiguienteContencion() {
  ...
  document.getElementById("lbl-contencion-title").textContent =
    `Validación: ${currentCat.name}`;
...
function evaluarTriggers(container) {
  container.querySelectorAll("[data-linked]").forEach((parentInput) => {
    ...
    const childInput = childGroup
      ? childGroup.querySelector("[data-qid]")
      : null;
    ...
    if (
      valorPadre !== "" &&
      valorPadre.toUpperCase() === triggerVal.toUpperCase()
    ) {
...
document.getElementById("form-preguntas").addEventListener("submit", (e) => {
  e.preventDefault();
  document
    .querySelectorAll("#container-preguntas-dinamicas [data-qid]")
    .forEach((inp) => {
    ...
    if (res.success) {
      if (huboRetrabajo) {
        ...
        pendientes[currentPiece.shopfloor_id] = {
          ...
          id_model: currentPiece.id_model,
          id_problem: retrabajoData.id_problem
            ? retrabajoData.id_problem.toString().trim()
            : "",
        };
        localStorage.setItem(CACHE_KEY_RETRABAJOS, JSON.stringify(pendientes));
      }
